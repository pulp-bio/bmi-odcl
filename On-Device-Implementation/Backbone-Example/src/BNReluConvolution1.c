...
static void load_weights_async(Layer tile, Layer body, Layer layer, TileIndex index) {
  ...
  dma_transfer_1d_async((DmaTransferConf) {
    .ext = layer.addr.scale + 24*index.output_channel,
    .loc = tile.addr.scale,
  ...
  dma_transfer_1d_async((DmaTransferConf) {
    .ext = layer.addr.bias + 24*index.output_channel,
    .loc = tile.addr.bias,
    .length_1d_copy = tile.weights.output_channel * 4,
    .dir = 1
  });
...
void __attribute__ ((noinline)) BNReluConvolution1(void *args) {
  ...
  DoubleBuffer db_input = {
    .addrs = { l1_buffer + 0, l1_buffer + 0 + 11400 },
    .index = 0
  };
  DoubleBuffer db_output = {
    .addrs = { l1_buffer + 22808, l1_buffer + 22808 + 11400 },
    .index = 0
  };
  DoubleBuffer db_weights = {
    .addrs = { l1_buffer + 45616, l1_buffer + 45616 + 816 },
    .index = 0
  };
  ...
  for(int iter=0; iter < total_tiles; iter++) {
    Address addr = {
      ...
      .scale = double_buffer_get_addr(db_weights) + 768,
      .bias = double_buffer_get_addr(db_weights) + 768 + 24,

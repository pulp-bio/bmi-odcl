...
static void load_weights_async(Layer tile, Layer body, Layer layer, TileIndex index) {
  dma_transfer_async((DmaTransferConf) {
    ...
    .length_1d_copy = (int) tile.weights.output_channel * 8 * 1*16 / 8,
    .hwc_to_chw = 0,
    .stride_2d = 16,
    .stride_1d = 1,
    .dir = 1
...
void __attribute__ ((noinline)) BNReluConvolution3(void *args) {
  ...
  const Layer layer = {
    .addr = {
      ...
      .scale = layer_args->L2_weights + 512,
      .bias = layer_args->L2_weights + 640,
  ...
  DoubleBuffer db_weights = {
    .addrs = { l1_buffer + 15184, l1_buffer + 15184 + 768 },
    .index = 0
  };
  ...
  for(int iter=0; iter < total_tiles; iter++) {
    Address addr = {
      ...
      .scale = double_buffer_get_addr(db_weights) + 512,
      .bias = double_buffer_get_addr(db_weights) + 512 + 128,

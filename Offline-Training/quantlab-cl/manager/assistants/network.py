...
class NetworkAssistant(object):
    ...
    def recv_networkmessage(self, networkmessage: NetworkMessage) -> None:
        ...
        try:
            self._network_pretrained = networkmessage.config['pretrained']
        except KeyError:
            self._network_pretrained = None
    ...
    def prepare(self, platform: PlatformManager, fold_id: int) -> Union[nn.Module, nn.DataParallel]:
        ...
        """
        if self._network_pretrained is None:
            ...
            if not self._pass_fold_id:
                net = self._network_class(**self._network_kwargs, seed=self._network_seeds[fold_id], pretrained=self._network_pretrained[fold_id])  # WARNING: all QuantLab networks require a seed now!
            else:
                net = self._network_class(**self._network_kwargs, seed=self._network_seeds[fold_id], fold_id=fold_id, pretrained=self._network_pretrained[fold_id])

...
def train_TL(args: argparse.Namespace):
    ...
    * during the *fold preamble* of the resumed training run, after
    ...
    # prepare assistants
    # data
    traindataassistant_1 = DataAssistant('train_1')
    ...
    traindataassistant_2 = DataAssistant('train_2')
    ...
    traindataassistant_3 = DataAssistant('train_3')
    ...
    traindataassistant_4 = DataAssistant('train_4')
    ...
    train_dataassistant_list = [traindataassistant_1, traindataassistant_2, traindataassistant_3, traindataassistant_4]
    valid_dataassistant_list = [validdataassistant_1, validdataassistant_2, validdataassistant_3, validdataassistant_4]

    '''
    # For dataset A, 7 sessions
    traindataassistant_5 = DataAssistant('train_5')
    ...
    traindataassistant_6 = DataAssistant('train_6')
    ...
    traindataassistant_7 = DataAssistant('train_7')
    ...
    for fold_id in range(start_fold_id, logbook.n_folds):
        for TL_id in range(0, num_TL):
            ...
            if TL_id != 0: # Note that, it's better to train the pretrained model fully, not only with the last layer!
                if args.freeze_except_last:
                    for name, param in net.named_parameters():
                        ...
                        print(param)
                        '''
                        # Unfreeze other parts
                        elif 'sep_conv2' in name:
                            param.requires_grad = True
                        elif 'sep_conv1' in name:
                            param.requires_grad = True
                        elif 'conv2' in name:
                            param.requires_grad = True
                        elif 'conv1' in name:
            ...
            if TL_id != num_TL - 1 and cur_epochs != 0:
                net.eval()
                # master-workers synchronisation point: quantization controllers might change the network's quantization parameters stochastically
                if (not platform.is_horovod_run) or platform.is_master:
                    for c in qnt_ctrls:
    ...
    for i in range(len(TL_pretrained_acc_mean)):

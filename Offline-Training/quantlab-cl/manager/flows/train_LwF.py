...
def train_LwF(args: argparse.Namespace):
    ...
    2. **Training loop**.
       ...
       * a *fold preamble*, during which the fold-specific loggingf
    ...
    * during the *fold preamble* of the resumed training run, after
    ...
    for fold_id in range(start_fold_id, logbook.n_folds):
        for TL_id in range(0, num_TL):
            ...
            # After loading state dict of net, start storing net_old!
            print("Loading last epoch_id: {}".format(start_epoch_id))
            net_old = copy.deepcopy(net)
            ...
            for epoch_id in range(start_epoch_id, cur_epochs):
                ...
                for batch_id, (x, ygt) in enumerate(train_loader):
                    ...
                    if TL_id == 0:
                        ...
                        loss = loss_fn(inputs=ypr, targets=ygt, is_first=is_first)
                        ...
                        # loss evaluation
                        ygt = ygt.to(platform.device)
                        loss = loss_fn(ypr, ygt, ypr_old, is_first)

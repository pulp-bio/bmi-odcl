...
class LogsManager(object):
    ...
    def store_checkpoint(self,
        ...
        if is_early_stop:
            ckpt_filename = self.make_ckpt_path('early_stopped.ckpt')
        else:
    ...
    def load_checkpoint(self,
                        ...
                        ckpt_id: Union[int, None] = None,
                        freeze_except_last: bool = False) -> int:

        ckpts_list = os.listdir(self._path_saves)

        if len(ckpts_list) > 0:  # a checkpoint exists

            if ckpt_id is None or ckpt_id == -2:  # discover most recent checkpoint; this mode is used when resuming crashed/interrupted experimental runs

...
class DataAssistant(object):
    ...
    def get_sampler(self,
        ...
        if platform.is_horovod_run:
            sampler = torch.utils.data.distributed.DistributedSampler(dataset, num_replicas=platform.global_size, rank=platform.global_rank, shuffle=True if self._partition == 'train' or self._partition == 'full_sess'  else False, seed=self._sampler_seeds[self._fold_id] if self._partition == 'train' or self._partition == 'full_sess'  else 0)
        else:
            generator = torch.Generator()
            generator.manual_seed(self._sampler_seeds[self._fold_id] if self._partition == 'train' or self._partition == 'full_sess'  else 0)
            sampler = torch.utils.data.RandomSampler(dataset, generator=generator) if self._partition == 'train' or self._partition == 'full_sess'  else torch.utils.data.SequentialSampler(dataset)
    ...
    def prepare_on_request_with_buffer(self, platform: PlatformManager, fold_id: int, group_size: int, buffer_indices: list, TL_id: int, dataset: Optional[torch.utils.data.Dataset] = None) -> torch.utils.data.DataLoader:
        """Create the training and validation/test ``DataLoader``s with ER buffer.
    ...
    def prepare_on_request_cur_test_selected_long_chain_all_test_acc(self, platform: PlatformManager, fold_id: int) -> torch.utils.data.DataLoader:
        """Create the validation/test ``DataLoader``s for TOR workflow for separate tests.
        ...
        # paste the printed TL train list here for testing

# 
# train_ER_on_request_withPretrained_prev_sessions_long_chain_test_acc.py
# 
# Author(s):
...
def train_ER_on_request_withPretrained_prev_sessions_long_chain_test_acc(args: argparse.Namespace):
    """Train a DNN or (possibly) a QNN in TOR framework with ER.

    This function implements QuantLab's training flow with TOR-ER. The training
    ...
    * during the *fold preamble* of the resumed training run, after
    ...
    # prepare assistants
    # data
    traindataassistant = DataAssistant('full_sess') # ER: including Session 1, all sessions in a chain.
    traindataassistant.recv_datamessage(logbook.send_datamessage('train'))
    validdataassistant_cur = DataAssistant('full_sess_val') # ER: except Session 1, all sessions in a chain.
    validdataassistant_cur.recv_datamessage(logbook.send_datamessage('valid'))
    validdataassistant = DataAssistant('valid') # validation data.
    ...
    buffer_size = 10 #10
    sess_1_size = 100 # 200 for 4 classes
    num_classes = 2
    ...
    num_groups = sess_1_size // group_size
    ...
    for fold_id in range(start_fold_id, logbook.n_folds):
        ...
        num_TL = len(valid_loader_list) - 1
        ...
        num_observed_samples = sess_1_size
        buffer_indices = np.arange(sess_1_size)
        np.random.shuffle(buffer_indices)
        buffer_indices = buffer_indices[:buffer_size].tolist()
        print(len(buffer_indices))

        TL_id = 0

        while TL_id <= num_TL:

            cur_trial_no = TL_id % num_groups #10

            print("TL_id: {}".format(TL_id))

            if TL_id == 0:
                train_loader, dataset = traindataassistant.prepare_on_request_with_buffer(platform, fold_id, group_size, buffer_indices, TL_id)
            elif TL_id != num_TL: 
                train_loader, _ = traindataassistant.prepare_on_request_with_buffer(platform, fold_id, group_size, buffer_indices, TL_id, dataset)
            ...
            valid_loader_before_train = valid_loader_list[TL_id]
            print(len(valid_loader_before_train))
            ...
            valid_meter  = validmeterassistant.prepare(platform, len(valid_loader_before_train), net)
            before_train_valid_meter = validmeterassistant_before.prepare(platform, len(valid_loader_before_train), net)
            ...
            if (not platform.is_horovod_run) or platform.is_master:
                logbook.logs_manager.create_writers(start_epoch_id=start_epoch_id, n_batches_train=len(train_loader), n_batches_valid=len(valid_loader_before_train))
            ...
            with torch.no_grad():
                for batch_id, (x, ygt) in enumerate(valid_loader_before_train):
            ...
            if cur_test_acc < T_thold and cur_trial_no != num_groups-1 and TL_id != num_TL:
                ...
                # Update buffer_indices

                for i in range(group_size):
                    num_observed_samples += 1
                    rand = np.random.randint(0, num_observed_samples)
                    if rand < buffer_size:
                        buffer_indices[rand] = num_classes*50 + TL_id*group_size+group_size+i 

                print("num_observed_samples: {}".format(num_observed_samples))   
    ...
    for i in range(5): # 5 folds
        ...
        for ind in range(len(TL_id_test_acc_list_each_fold[i])):
            ...
            cur_sess_no = cur_TL_id // num_groups
            total_acc_per_fold_for_each_sess[cur_sess_no].append(cur_acc)
            cur_fold_test_trials[cur_sess_no] += 1
        for j in range(n_sess):
            total_train_trials_per_fold_for_each_sess[j].append((num_groups-cur_fold_test_trials[j])*group_size)
    ...
    print(total_acc_per_fold_for_each_sess[4])

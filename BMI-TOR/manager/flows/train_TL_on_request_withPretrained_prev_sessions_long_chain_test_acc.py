# 
# train_TL_on_request_withPretrained_prev_sessions_long_chain_test_acc.py
# 
# Author(s):
...
def train_TL_on_request_withPretrained_prev_sessions_long_chain_test_acc(args: argparse.Namespace):
    """Train a DNN or (possibly) a QNN in TOR framework with TL.
    ...
    * during the *fold preamble* of the resumed training run, after
    ...
    for fold_id in range(start_fold_id, logbook.n_folds):
        ...
        while TL_id <= num_TL:
            ...
            if (not platform.is_horovod_run) or platform.is_master:
                logbook.logs_manager.create_writers(start_epoch_id=start_epoch_id, n_batches_train=len(train_loader), n_batches_valid=len(train_loader))
            ...
            if cur_test_acc < T_thold and cur_trial_no != 9 and TL_id != num_TL:
                ...
                cur_TL_train.append(TL_id+1) # No. of sessions that is used as training set. range: No. 0-59 (10 per subsession)
    ...
    for i in range(n_sess):
        avg_acc_per_fold_for_each_sess[i] = np.average(np.array(total_acc_per_fold_for_each_sess[i])) #total_acc_per_fold_for_each_sess[i] / total_test_trials_per_fold_for_each_sess[i]
        avg_train_trials_per_fold_for_each_sess[i] = np.average(np.array(total_train_trials_per_fold_for_each_sess[i])) # total_test_trials_per_fold_for_each_sess[i] / 5

...
# Lan: select best checkpoint with the best validation accuracy (task statistic)
...
class Meter(object):
    ...
    @property
    def cur_loss_statistic(self) -> bool:
        return self._loss_statistic.value.item()

    @property
    def cur_task_statistic(self) -> bool:
        return self._task_statistic.value.item()

    def check_improvement(self):
        if self._task_statistic.value.item() > self.best_acc:
            self._best_acc = self._task_statistic.value.item()
            self._is_best   = True
        else:
            self._is_best = False

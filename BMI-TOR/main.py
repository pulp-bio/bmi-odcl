...
from manager.flows import train_TL_on_request_withPretrained_prev_sessions_long_chain_test_acc
from manager.flows import train_ER_on_request_withPretrained_prev_sessions_long_chain_test_acc
from manager.flows import train_LwF_on_request_withPretrained_prev_sessions_long_chain_test_acc
from manager.flows import test
from manager.flows import test_multi
from manager.flows import test_multi_select_TOR_long_chain_check_all_test_acc
...
def parse_cli():
    ...
    parser_train.add_argument('--ckpt_id',  required=False, type=int, default=-2,                                    help="The ID of the checkpoint (best: -1) to launch or resume")
    parser_train.add_argument('--horovod', required=False, action='store_true',                                      help="Distributed data parallel training with Horovod")
    parser_train.add_argument('--freeze_except_last', required=False, action='store_true',                           help="If freezing the layers in network except for the last layer or not")
    parser_train.add_argument('--early_stopping', required=False, action='store_true',                               help="Early stopping or not")
    parser_train.add_argument('--patience',  required=False, type=int, default=10,                                   help="Patience of early-stopping")
    parser_train.set_defaults(func=train)
    # training flow - TOR-TL
    parser_train = subparsers.add_parser('train_TL_on_request_withPretrained_prev_sessions_long_chain_test_acc',                                                                    help="Launch a training or fine-tuning experimental run")
    parser_train.add_argument('--exp_id',  required=True,  type=int,                                                 help="The ID of the experimental run to launch or resume")
    parser_train.add_argument('--ckpt_id',  required=False, type=int, default=-2,                                    help="The ID of the checkpoint (best: -1) to launch or resume")
    parser_train.add_argument('--horovod', required=False, action='store_true',                                      help="Distributed data parallel training with Horovod")
    parser_train.add_argument('--freeze_except_last', required=False, action='store_true',                           help="If freezing the layers in network except for the last layer or not")
    parser_train.add_argument('--early_stopping', required=False, action='store_true',                               help="Early stopping or not")
    parser_train.add_argument('--patience',  required=False, type=int, default=10,                                   help="Patience of early-stopping")
    parser_train.set_defaults(func=train_TL_on_request_withPretrained_prev_sessions_long_chain_test_acc)
    # training flow - TOR-ER
    parser_train = subparsers.add_parser('train_ER_on_request_withPretrained_prev_sessions_long_chain_test_acc',                                                                    help="Launch a training or fine-tuning experimental run")
    parser_train.add_argument('--exp_id',  required=True,  type=int,                                                 help="The ID of the experimental run to launch or resume")
    parser_train.add_argument('--ckpt_id',  required=False, type=int, default=-2,                                    help="The ID of the checkpoint (best: -1) to launch or resume")
    parser_train.add_argument('--horovod', required=False, action='store_true',                                      help="Distributed data parallel training with Horovod")
    parser_train.add_argument('--freeze_except_last', required=False, action='store_true',                           help="If freezing the layers in network except for the last layer or not")
    parser_train.add_argument('--early_stopping', required=False, action='store_true',                               help="Early stopping or not")
    parser_train.add_argument('--patience',  required=False, type=int, default=10,                                   help="Patience of early-stopping")
    parser_train.set_defaults(func=train_ER_on_request_withPretrained_prev_sessions_long_chain_test_acc)
    # training flow - TOR-LwF
    parser_train = subparsers.add_parser('train_LwF_on_request_withPretrained_prev_sessions_long_chain_test_acc',                                                                    help="Launch a training or fine-tuning experimental run")
    parser_train.add_argument('--exp_id',  required=True,  type=int,                                                 help="The ID of the experimental run to launch or resume")
    parser_train.add_argument('--ckpt_id',  required=False, type=int, default=-2,                                    help="The ID of the checkpoint (best: -1) to launch or resume")
    parser_train.add_argument('--horovod', required=False, action='store_true',                                      help="Distributed data parallel training with Horovod")
    parser_train.add_argument('--freeze_except_last', required=False, action='store_true',                           help="If freezing the layers in network except for the last layer or not")
    parser_train.add_argument('--early_stopping', required=False, action='store_true',                               help="Early stopping or not")
    parser_train.add_argument('--patience',  required=False, type=int, default=10,                                   help="Patience of early-stopping")
    parser_train.set_defaults(func=train_LwF_on_request_withPretrained_prev_sessions_long_chain_test_acc)
    # testing flow
    ...
    parser_test.add_argument('--fold_id', required=False,  type=int, default=0,                                      help="The ID of the cross-validation fold to which the trained model belongs")
    ...
    # testing flow - Multiple sessions
    parser_test = subparsers.add_parser('test_multi',                                                                help="Assess the performance of a trained model on test data")
    parser_test.add_argument('--exp_id',  required=True,  type=int,                                                  help="The ID of the experimental unit to which the trained model belongs")
    parser_test.add_argument('--fold_num', required=False,  type=int, default=0,                                     help="The ID of the cross-validation fold to which the trained model belongs")
    parser_test.add_argument('--ckpt_id', required=False, nargs='+', type=int,                                       help="The trained model checkpoint's file ID (default: best model)")
    parser_test.add_argument('--horovod', required=False, action='store_true',                                       help="Distributed data parallel test with Horovod")
    parser_test.set_defaults(func=test_multi)
    # testing flow - Multiple sessions using TOR
    parser_test = subparsers.add_parser('test_multi_select_TOR_long_chain_check_all_test_acc',                                                                      help="Assess the performance of a trained model on test data")
    parser_test.add_argument('--exp_id',  required=True,  type=int,                                                  help="The ID of the experimental unit to which the trained model belongs")
    parser_test.add_argument('--fold_num', required=False,  type=int, default=0,                                     help="The ID of the cross-validation fold to which the trained model belongs")
    parser_test.add_argument('--ckpt_id', required=False, nargs='+', type=int,                                       help="The trained model checkpoint's file ID (default: best model)")
    parser_test.add_argument('--horovod', required=False, action='store_true',                                       help="Distributed data parallel test with Horovod")
    parser_test.set_defaults(func=test_multi_select_TOR_long_chain_check_all_test_acc)
    # quantization flow
